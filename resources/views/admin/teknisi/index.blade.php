...
<body class="bg-light text-dark">
    <div class="container py-5 px-3 px-md-5">
        ...
        <div class="card mb-5">
            ...
            <div class="card-body">
                <form action="{{ route('teknisi.store') }}" method="POST" class="row g-3">
                    @csrf
                    <div class="col-md-6">
...
<table class="table table-bordered table-sm align-middle">
    ...
    <tbody>
        ...
        <tr class="align-top">
            <td>
                <form action="{{ route('teknisi.update', $user->id) }}" method="POST" class="d-flex flex-column gap-2">
                    @csrf
                    @method('PUT')
            ...
            <td>
                    <input type="text" name="telepon" value="{{ $user->telepon }}" class="form-control form-control-sm">
            </td>
            <td>
                    <input type="text" name="alamat" value="{{ $user->alamat }}" class="form-control form-control-sm">
            </td>
            <td class="text-center">
            <div class="d-flex flex-column gap-1">
                    <button type="submit" class="btn btn-warning btn-sm w-100">Update</button>
                </form>
                <form action="{{ route('teknisi.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus teknisi ini?')">
                    @csrf
                    @method('DELETE')
        ...
        @endforeach
                ...
                <!-- Paginasi -->
                <div class="mt-3">
                    {{ $teknisis->appends(request()->query())->links('pagination::bootstrap-5') }}
                </div>
            </div>
        </div>
    </div>
